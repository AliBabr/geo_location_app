class Api::V1::UsersController < ApplicationController
  ...
  def sign_in
    if params[:email].blank?
      ...
      if user.present? && user.valid_password?(params[:password])
        ...
        if user.role = "Coach"
          render json: { email: user.email, name: user.name, phone: user.phone, profile_photo: image_url, city: user.city, about: user.about, background: user.background, category: user.category, coach_type: user.coach_type, "UUID" => user.id, "Authentication" => user.authentication_token }, status: 200
        else
          render json: { email: user.email, name: user.name, phone: user.phone, profile_photo: image_url, city: user.city, "UUID" => user.id, "Authentication" => user.authentication_token }, status: 200
        end
      else
  ...
  def get_user
    image_url = ""
    image_url = url_for(@parsm_user.profile_photo) if @param_user.profile_photo.attached?
    if user.role = "Coach"
      render json: { email: user.email, name: user.name, phone: user.phone, profile_photo: image_url, city: user.city, about: user.about, background: user.background, category: user.category, coach_type: user.coach_type, "UUID" => user.id, "Authentication" => user.authentication_token }, status: 200
    else
      render json: { email: user.email, name: user.name, phone: user.phone, profile_photo: image_url, city: user.city, "UUID" => user.id, "Authentication" => user.authentication_token }, status: 200
  ...
  def sign_up
    ...
    if user.save
      image_url = ""
      image_url = url_for(user.profile_photo) if user.profile_photo.attached?
      render json: { email: user.email, name: user.first_name, last_name: user.last_name, profile_photo: image_url, pickup_notification: user.pickup_notification, arrival_notification: user.arrival_notification, "UUID" => user.id, "Authentication" => user.authentication_token }, status: 200
    else
