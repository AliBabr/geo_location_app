class Api::V1::UsersController < ApplicationController
  ...
  def get_user
    ...
    image_url = url_for(@user.profile_photo) if @user.profile_photo.attached?
    if @user.role == "Coach"
      render json: { user_id: @user.id, email: @user.email, name: @user.name, phone: @user.phone, profile_photo: image_url, city: @user.city, about: @user.about, background: @user.background, category: @user.category, type: @user.leason_type, role: @user.role, video_calling_id: @user.video_calling_id }, status: 200
    else
  ...
  def add_video_calling_id
    ...
    if @user.errors.any?
      ...
      image_url = url_for(@user.profile_photo) if @user.profile_photo.attached?
      if @user.role == "Coach"
        render json: { current_user_id: @user.id, email: @user.email, name: @user.name, phone: @user.phone, profile_photo: image_url, city: @user.city, video_calling_id: @user.video_calling_id, about: @user.about, background: @user.background, category: @user.category, type: @user.leason_type }, status: 200
      else
        render json: { current_user_id: @user.id, email: @user.email, name: @user.name, phone: @user.phone, profile_photo: image_url, city: @user.city , video_calling_id: @user.video_calling_id }, status: 200
      end
    end
  rescue StandardError => e
  ...
  def user_params # permit user params
    params.permit(:email, :password, :name, :city, :profile_photo, :phone, :about, :background, :role, :video_calling_id)
  end
